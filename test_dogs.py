...
# 1. Рандомная проверка -- сложности воспроизведения и отладки, неполное покрытие сценариев, т.к. будет вызван только один из кейсов, а второй -- нет. 
#    parametrize должен задавать оба сценария как 2 отдельных кейса внутри одного теста. (Или сделать 2 отдельных теста, но не вижу бенефитов, пьюристы восхитятся)
# 2. У запросов нет проверок на статус коды, что при ошибке приведет к парсингу неожиданного response body и соотв рантайм ошибке.
...
# 5. Класс YaUploader по сути не нужен в текущем виде, т.к. не связывает и не инкаспсулирует ничего, все методы могут быть статическими -> клас не нужен.
#    Варианты:
#    1) (мне больше нравится) перенсти общие части/зависимости в поля/методы класса; тогда класс будет не_бесполезен и будет выполнять ООП задачи
...
# 6. С другой стороны, работа с собачьим АПИ не абстарктизирована (я знаю, что там 1 метод, но есть смысл закладывать архитектуру раньше),
#    Предлагаю сделать класс как в 5->1
# 7. Методы YaUploader слишком прямо взаимодействуют с запросами -- хедеры, бади, етс. Также повторяются ссылки, хедеры, парсинг ошибок (если бы он был).
...
#    
# 8. Засвечен токен. Для тествого задания тестовый токен не катастрофа, но реального его нужно брать из
# - файла (гитигнорного чтоб не закомментить в репу)
# - переменной среды (задавть посредством CI раннера) (оптимальный вариант ИМО)
# - хранилища секретов по АПИ
# 9. Метод U имеет старнный нейминг  и слишком много responsibilty. 
# 10. Создание YaUploader в U нарушает Dependecy Injection (soliD)
# 11. Тест не очищает мусор за собой, есть смысл юзать фикстуру (например, контекстный менджер в YaUploader() который будет в __exit__() убирать за собой
# 12. Тест  активно работает с сырыми словарями. Лучше получать из методов объекты
# 13. Это однофайльный тест, но в реальном приложении классы YaUploader и прочая должны быть вынесены  в отедльную папку с фреймворком, в тестах должы быть только тесты.
#     Опять же, для такого маленького теста файла нет смысла заниматься таким переисыванием.
# 14. В u() -> for ulr in urls: происходит что-то непонятное, нужен коммент или вынос в функцию с содержательынм именем.
# 15. Несмотря на наличие YaUploader класса, который должен абстрагировать и реализовывать всю работа с диском в себе, тест напрямую ходит в АПИ.
