...
# *. Рандомная проверка -- сложности воспроизведения и отладки, неполное покрытие сценариев,
...
# *. У запросов нет проверок на статус коды,
...
# *. Тесты не по документации АПИ -- ресурсы могут создаваться асинхронно, требуется ждать их появления.

# *. Постоянное дерганье собачьего АПИ может исчерпать лимиты, надо кешировать результаты.

# *. Даже с кешем есть ограничения со стороны АПИ на кол-во запросов.
#    Не стал выяснять где именно, добавил ретрай для вызова метода загрузки со стороны Яндекса,
#    т.к. полагаю что Яндекс АПИ получает рейт лимит со стороны Dog API.

# *. Нет док-стрингов, нужно открывать функцию чтоб понять что происходит

# *. Нет тайп хинтов (type annotations) на аргументы функций  и возврат функций (def f_1(arg: int, ......) -> SomeType)

# *. Класс YaUploader по сути не нужен в текущем виде,
...
# *. С другой стороны, работа с собачьим АПИ не абстарктизирована
#   (я знаю, что там 1 метод, но есть смысл закладывать архитектуру раньше).
#    Предлагаю сделать класс как в 5->1

# *. Методы YaUploader слишком прямо взаимодействуют с запросами -- хедеры, бади, етс.
...
# *. Засвечен токен. Для тестового задания тестовый токен не катастрофа, но реального его нужно брать из
...
# *. Метод U имеет странный  нейминг и слишком много responsibilty, будут архитектурные трудности.

# *. Создание YaUploader в U нарушает Dependency Injection (soliD)

# *. Тест не очищает мусор за собой, есть смысл юзать фикстуру
#     (например, контекстный менджер в YaUploader() который будет в __exit__() убирать за собой

# *. Тест активно работает с сырыми словарями. Лучше получать из методов объекты

# *. Это однофайльный тест, но в реальном приложении классы YaUploader и прочая должны быть вынесены в отельную папку
...
# *. В u() -> for ulr in urls: происходит что-то непонятное, нужен коммент или вынос в функцию с содержательным именем.

# *. Несмотря на наличие YaUploader класса, который должен абстрагировать и реализовывать всю работу с диском в себе,
#     тест напрямую ходит в АПИ.

# *. f-строки для постарения параметров в реквестах, можно использовать params вместо.

# *. assert True :D

# * [Опциональный] assert без сообщений о том что упало. Не во всех коллективах это считается плохо.

# *. test_proverka -- bad English.
